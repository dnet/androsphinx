...
private val SYMBOL_OFFSET = CharacterClass.values().size + RULE_SHIFT
...
data class Rule(val charClasses: Set<CharacterClass>, val symbols: Set<Char>,
                val size: BigInteger, val checkDigit: BigInteger, val xorMask: BigInteger) {
    fun serialize(): ByteArray {
        ...
        return withCheckDigit.shiftLeft(XOR_MASK_BYTES * 8).or(xorMask).toByteArray()
    }
    ...
    companion object {
        fun parse(serialized: ByteArray): Rule {
            ...
            return Rule(charClasses, symbols, size, checkDigit, xorMask)
        }
    }
}
